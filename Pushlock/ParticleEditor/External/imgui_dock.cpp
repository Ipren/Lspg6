...
struct DockContext
{
	...
	bool dockSlots(Dock& dock, Dock* dest_dock, const ImRect& rect, bool on_border)
	{
		...
		ImU32 color = GetColorU32(ImGuiCol_TitleBgCollapsed);
		ImU32 color_hovered = GetColorU32(ImGuiCol_ButtonHovered);
		...
		for (int i = 0; i < (on_border ? 4 : 5); ++i)
		{
			...
			canvas->AddRectFilled(docked_rect.Min, docked_rect.Max, GetColorU32(ImGuiCol_TitleBgCollapsed));
		}
		return false;
	}
	...
	{
		float tabbar_height = 2 * GetTextLineHeightWithSpacing() + 2;
		...
		if (BeginChild(tmp, size, true))
		{
			...
			while (dock_tab)
			{
				...
				ImVec2 size = ImVec2(CalcTextSize(dock_tab->label, text_end).x, line_height) + ImVec2(16, 0);
